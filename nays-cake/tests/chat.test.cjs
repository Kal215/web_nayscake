...
test('reported greeting, permission and menu conversation stays on topic without AI calls', async () => {
  const rows = Array.from({ length: 7 }, (_, i) => ({ id: 'p' + i, name: 'Kue ' + i, sellingPrice: 1500 + i * 100, unit: 'pcs', supplier: { name: 'MITRA INTERNAL' } }));
  let reads = 0;
  const ai = loader({ '@/lib/prisma': { prisma: { product: { findMany: async () => { reads++; return rows; } } } } })('src/lib/chat-ai.ts');
  const history = [];
  async function ask(content) {
    history.push({ role: 'VISITOR', content });
    const reply = await ai.answerChat(history);
    history.push({ role: 'AI', content: reply.content });
    assert.equal(reply.handoff, false);
    return reply.content;
  }
  assert.match(await ask('hai'), /selamat datang/);
  const permission = await ask('saya ingin bertanya seputar kue, apa bisa ya?');
  assert.match(permission, /Bisa, Kak/); assert.doesNotMatch(permission, /selamat datang/);
  assert.equal(reads, 0);
  const menu = await ask('tolong menu nya ada apa saja');
  assert.match(menu, /7 pilihan produk/); assert.match(menu, /beberapa di antaranya/);
  assert.match(menu, /https:\/\/nayscake.vercel.app\/catalog/);
  assert.equal((menu.match(/^- Kue/gm) || []).length, 5);
  assert.match(menu, /Rp 1.500/); assert.doesNotMatch(menu, /MITRA INTERNAL/);
  assert.equal(reads, 1);
  assert.match(await ask('hai'), /Halo lagi/);
});

test('social shortcuts do not swallow specific questions or safety requests', async () => {
  let calls = 0;
  const ai = loader({ '@/lib/prisma': { prisma: { product: { findMany: async () => [] } } }, '@/lib/chat-security': { rateLimit: async () => {} }, '@/lib/chat-providers': {
    chatProviders: () => [{ name: 'test' }], providerSelection: async () => { calls++; return { kind: 'facts', ids: ['jam_utama'] }; },
  } })('src/lib/chat-ai.ts');
  for (const question of ['Halo, jam buka toko utama?', 'saya ingin bertanya seputar kue, harganya berapa?', 'menu risol berapa?']) {
    await ai.answerChat([{ role: 'VISITOR', content: question }]);
  }
  assert.equal(calls, 3);
  assert.equal((await ai.answerChat([{ role: 'VISITOR', content: 'hai, menu untuk alergi?' }])).handoff, true);
  assert.equal(calls, 3);
});

test('irrelevant greeting from provider is rejected and fallback receives conversation context', async () => {
  const history = [{ role: 'VISITOR', content: 'Cabang Rancapanggung di mana?' }, { role: 'AI', content: 'Google Maps cabang Rancapanggung' }, { role: 'VISITOR', content: 'Kalau jam bukanya?' }];
  let calls = 0;
  const ai = loader({ '@/lib/prisma': { prisma: { product: { findMany: async () => [] } } }, '@/lib/chat-security': { rateLimit: async () => {} }, '@/lib/chat-providers': {
    chatProviders: () => [{}, {}], providerSelection: async (_provider, prompt, received) => {
      assert.equal(received, history); assert.match(prompt, /jangan menjawab ulang/);
      return ++calls === 1 ? { kind: 'greeting', ids: [] } : { kind: 'facts', ids: ['jam_cabang'] };
    },
  } })('src/lib/chat-ai.ts');
  const reply = await ai.answerChat(history);
  assert.equal(calls, 2); assert.match(reply.content, /07.00-12.00/); assert.equal(reply.handoff, false);
});

test('catalog handles empty data, ambiguous names and invalid references', () => {
  const { renderSelection } = loader()('src/lib/chat-ai.ts');
  assert.equal(renderSelection({ kind: 'catalog', ids: [] }, []).handoff, true);
  const products = ['A', 'B'].map((supplier, i) => ({ id: String(i), name: 'Risol', sellingPrice: 1500, unit: 'pcs', supplier }));
  const reply = renderSelection({ kind: 'catalog', ids: [] }, products);
  assert.match(reply.content, /Risol \(A\)/); assert.match(reply.content, /Risol \(B\)/);
  assert.doesNotMatch(reply.content, /beberapa di antaranya/);
  assert.throws(() => renderSelection({ kind: 'catalog', ids: ['0'] }, products));
  assert.throws(() => renderSelection({ kind: 'catalog', ids: [] }, [{ ...products[0], sellingPrice: NaN }]));
  assert.match(renderSelection({ kind: 'inquiry', ids: [] }, []).content, /Bisa, Kak/);
  assert.equal(renderSelection({ kind: 'clarify', ids: [] }, []).handoff, false);
});
