...
export default function DashboardPage() {
  ...
  return <Sidebar><main className="max-w-7xl mx-auto space-y-6">
    <div className="flex justify-between items-center"><h1 className="text-2xl font-bold">Dashboard</h1><button title="Muat ulang" aria-label="Muat ulang" onClick={() => setRefresh(n => n + 1)} className="p-2 border rounded-lg"><RefreshCw size={20} /></button></div>
    {error && <p role="alert" className="text-red-700">{error}</p>}
    {!data && !error && <p role="status">Memuat dashboard...</p>}
    {data && <>
      <dl className="grid grid-cols-2 xl:grid-cols-4 gap-6 border-b pb-6">
        {[["Produk Aktif", data.products], ["Supplier Aktif", data.suppliers], ["Omzet Hari Ini", money(data.totals.revenue)], ["Laba Hari Ini", money(data.totals.profit)]].map(([label, value]) => <div key={label}><dt className="text-sm text-gray-600">{label}</dt><dd className="text-lg font-semibold mt-2 break-words">{value}</dd></div>)}
      </dl>
